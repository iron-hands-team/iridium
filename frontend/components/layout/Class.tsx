...
export default function ClassBody() {
  ...
  function updateName(index: number, text: string) {
    setClasses((prev) =>
      prev.map((item, i) =>
        i === index ? { ...item, className: text } : item,
      ),
    );
  }
    ...
    setClasses((prev) =>
      prev.map((item, i) =>
        i === index
          ? {
              ...item,
              students: subUpdateStudentName(item.students, studentIndex, text),
            }
          : item,
      ),
    );
  ...
  function subUpdateStudentName(
    students: Test[],
    studentIndex: number,
    text: string,
  ) {
    return students.map((student, i) =>
      i === studentIndex ? { ...student, name: text } : student,
    );
  }
  ...
  return (
    <div className="bg-[#131313]">
      ...
      >
        ...
        <p>
          {classes.length == 0
            ? "No classes to show. Create one on the top-left!"
            : ""}
        </p>
        {classes.map((item, i) => (
          <div
          ...
          >
            ...
            <div className="w-2/3 flex">
              <Btn
                text="Add Student"
                onclick={() => {
                  classes[i].canShow = true;
                  addStudent(item.students, i);
                }}
              />
              <Btn
                text={classes[i].canShow ? "Hide" : "Show"}
                onclick={() => updateCanShowStudents(i, !classes[i].canShow)}
              />
            </div>
            {classes[i].canShow && (
              <div>
                ...
                <p>
                  {classes[i].students.length == 0
                    ? "No students in your class currently."
                    : ""}
                </p>
                    ...
                    >
                      <div className="w-2/3">
                        ...
                        <textarea
                          ...
                          className="text-white dark:text-zinc-300 text-sm w-2/3 resize-none h-15 bg-[#333333] p-2 mt-2"
                          value={studentTest.name}
                          onChange={(e) => {
                            updateStudentName(i, j, e.target.value);
                          }}
                        ></textarea>
                      </div>
